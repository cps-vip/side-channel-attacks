...
Power consumption leaks information - 
   When a microcontroller processes data, transistors switch
   and each switch costs energy. 
   So instantaneous power draw is proportional to number of bits flipping.
   This is the Hamming weight model.
   As we don't have access to the phyiscal power draw,
...
# Calculating hamming weights of bytes
...
def generate_traces(num_traces, noise_std):
    ...
    """
    # Generate random key
    key = np.random.randint(0, 256, size=NUM_BYTES, dtype=np.uint8)
    
    # Generate random plaintexts
    plaintexts = np.random.randint(0, 256, size=(num_traces, NUM_BYTES), dtype=np.uint8)
    
    # Compute intermediate values  ->  SBOX[plaintext ^ key]
    intermediate = plaintexts ^ key
