...
const SAMPLE_SWEEP: &[usize] = &[1, 10, 20, 50, 100, 200];
...
#[inline(never)]
fn trimmed_mean_time(attempt: &str, samples: usize) -> u64 {
...
fn attack(samples: usize) -> AttackStats {
    let attack_start = Instant::now();
    let mut known = String::new();
    let mut gap_sum: u128 = 0;
    let mut gap_ratio_sum: f64 = 0.0;
    ...
    for pos in 0..SECRET.len() {
        let mut times: Vec<(char, u64)> = CHARSET
            ...
            .map(|c| {
                ...
                (c, trimmed_mean_time(&attempt, samples))
            })
            .collect();
        ...
        println!("pos {}: best='{}' correct='{}' gap={} ns correct={}", pos, best_char, correct_char, gap, was_correct);
    }
    ...
    println!("guessed: {} success: {}", known, success);
...
fn append_run_to_csv(path: &str, samples: usize, run_num: usize, stats: &AttackStats) -> Result<()> {
    let mut file = OpenOptions::new().create(true).append(true).open(path)?;
    writeln!(file, "{},{},{},{},{},{},{},{}", ITERS, samples, run_num, stats.guessed_password, stats.success, stats.runtime_ns, stats.avg_gap_ns, stats.avg_gap_ratio)?;
    Ok(())
}

fn append_position_stats_to_csv(path: &str, samples: usize, run_num: usize, stats: &AttackStats) -> Result<()> {
    let mut file = OpenOptions::new().create(true).append(true).open(path)?;
    for ps in &stats.position_stats {
        writeln!(file, "{},{},{},{},{},{},{},{},{},{},{},{},{}", ITERS, samples, run_num, ps.position, ps.chosen_char, ps.correct_char, ps.chosen_score_ns, ps.second_char, ps.second_score_ns, ps.gap_ns, ps.gap_ratio, ps.was_correct, ps.partial_guess)?;
    }
...
fn main() -> Result<()> {
    let attack_csv_path = "sample_sweep_stats.csv";
    let position_csv_path = "sample_sweep_position_stats.csv";
    ...
    for &samples in SAMPLE_SWEEP {
        ...
        for run in 1..=RUNS {
            println!("run {} / {} (samples={})", run, RUNS, samples);
            let stats = attack(samples);
            append_run_to_csv(attack_csv_path, samples, run, &stats)?;
            append_position_stats_to_csv(position_csv_path, samples, run, &stats)?;
        }
    }
